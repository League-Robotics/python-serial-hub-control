...
class Client:
    ...
    def __init__(self):
        ...
        self.serial: RHSPSerial | None = None

    def open(self, port: comPort | None = None):
        if port is None:
            port = comPort.enumerate()[0]

        if self.serial is not None:
            self.close()

        self.serial = RHSPSerial(port.name, **self._serial_config)
    ...
    def sendAndReceive(self, PacketToWrite, destination):
        ...
        beenInLoop = False
        lengthBytes_str = ''  # Initialize the variable
        try:
            retryAttempt = 0
            while retry:
                ...
                if isinstance(PacketToWrite, REVMsg.REVPacket):
                    ...
                    self.serial.write(binascii.unhexlify(PacketToWrite.getPacketData()))
                    ...
                    if self.serial.inWaiting() > 0:
                        while self.serial.inWaiting() > 0:
                            retry = False
                            newByte = binascii.hexlify(self.serial.read(1)).upper()
                            newByte = str(newByte)
                            newByte = newByte[2:]
                            newByte = newByte[:-1]
                            if parseState == self.WaitForFrameByte1:
                                ...
                                lengthBytes_str = newByte
                                parseState = self.WaitForPacketLengthByte2
                            elif parseState == self.WaitForPacketLengthByte2:
                                incomingPacket += newByte
                                lengthBytes_str += newByte
                                lengthBytes_int = int(int(lengthBytes_str, 16) >> 8 | int(lengthBytes_str, 16) % 256 << 8)
                                if lengthBytes_int <= REVMsg.PAYLOAD_MAX_SIZE:
                                    packetLength = lengthBytes_int
    ...
    def checkResponse(self, receivedPacket, PacketToWrite):
        ...
        if packetType == responseExpected:
            ...
            if packetType == REVMsg.MsgNum.NACK:
                ...
                print('Incorrect Response Type. Response Expected: ', hex(data), ', Response Received: ', hex(packetType))
                return False
